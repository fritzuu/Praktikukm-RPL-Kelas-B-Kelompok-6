...
export default function DosenAiAssistantPanel({ isOpen, onClose, ref }) {
    ...
    async function handleSend() {
        ...
        try {
            ...
            const res = await fetch(route('dosen.aiQuery'), {
                method: 'POST',
                headers: {
            ...
            if (!messageAdded) {
                setMessages(prev => [...prev, {
                    role: 'assistant',
                    text: 'Maaf Bapak/Ibu, saya tidak bisa memproses pertanyaan Anda saat ini. Silakan coba lagi.',
                }]);
            }
        } catch (err) {
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span className="text-[10px] font-semibold text-success uppercase tracking-wide">
                                {isStreaming ? 'Thinking...' : 'Lecturer Mode'}
                            </span>
                        </div>
                    </div>
                    <button
                ...
                <div className="px-4 py-2 border-t border-border">
                    <div className="flex gap-1.5 overflow-x-auto">
                        {[
                            ...
                            'Ruangan untuk kelas saya?'
                        ].map((q, i) => (
                            <button
                                key={i}
                                onClick={() => { setChatInput(q); }}
                                className="text-[10px] font-medium text-primary-500 bg-primary-500/5 hover:bg-primary-500/10 px-2.5 py-1.5 rounded-lg whitespace-nowrap transition-colors animate-fade-in"
                            >
                                {q}
                            </button>
                        ))}
