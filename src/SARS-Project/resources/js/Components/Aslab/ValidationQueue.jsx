...
import { ClipboardCheck, CheckCircle, XCircle, User, Calendar, Clock, MapPin, FileText, ChevronDown, ChevronUp } from 'lucide-react';

export default function ValidationQueue({ pending = [], recent = [], onForward, onReject }) {
    ...
    const handleForward = (id) => {
        if (onForward) {
            onForward(id, forwardNotes || 'Diteruskan ke Admin untuk keputusan akhir.');
        }
        setForwardNotes('');
    };
            ...
            <div>
                ...
                {pending.length === 0 ? (
                    <div className="bg-card border border-border rounded-2xl p-12 text-center">
                        ...
                        <h3 className="text-lg font-bold text-text-primary">Semua Tervalidasi</h3>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div className="bg-surface/50 rounded-xl p-4 border border-border/50">
                                                <div className="flex items-center gap-2 mb-2">
                                                    ...
                                                    <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider">Jadwal Diajukan</span>
                                                </div>
                                            ...
                                            <div className="bg-surface/50 rounded-xl p-4 border border-border/50">
                                                <div className="flex items-center gap-2 mb-2">
                                                    ...
                                                    <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider">MK & Ruangan</span>
                                                </div>
                                                <p className="text-sm font-semibold text-text-primary">
                                                    {item.schedule.course}
                                                </p>
                                                <p className="text-xs text-text-muted mt-0.5">
                                                    Ruangan: {item.schedule.room}
                                                </p>
                                            </div>
                                        </div>
            ...
            {recent.length > 0 && (
                <div>
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h2 className="text-lg font-bold text-text-primary">Riwayat Validasi</h2>
                    </div>
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-surface/50 border-b border-border">
                                        <th className="px-5 py-3 text-left text-[10px] font-bold text-text-muted uppercase tracking-widest">Mahasiswa</th>
                                        <th className="px-5 py-3 text-left text-[10px] font-bold text-text-muted uppercase tracking-widest">Mata Kuliah</th>
                                        <th className="px-5 py-3 text-left text-[10px] font-bold text-text-muted uppercase tracking-widest">Ruangan</th>
                                        <th className="px-5 py-3 text-left text-[10px] font-bold text-text-muted uppercase tracking-widest">Keputusan</th>
                                        <th className="px-5 py-3 text-left text-[10px] font-bold text-text-muted uppercase tracking-widest">Waktu</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {recent.map((item) => (
                                        <tr key={item.id} className="border-b border-border last:border-b-0 hover:bg-surface/20 transition-colors">
                                            <td className="px-5 py-3.5">
                                                <p className="text-sm font-semibold text-text-primary">{item.student}</p>
                                                <p className="text-[11px] text-text-muted">{item.requestCode}</p>
                                            </td>
                                            <td className="px-5 py-3.5 text-sm text-text-secondary">{item.course}</td>
                                            <td className="px-5 py-3.5 text-sm text-text-secondary">{item.room}</td>
                                            <td className="px-5 py-3.5">
                                                <span className={`text-[10px] font-bold px-2.5 py-1 rounded-md uppercase ${
                                                    ...
                                                    {item.decision === 'FORWARDED' ? '✓ Diteruskan' : '✗ Ditolak'}
                                                </span>
                                            </td>
                                            <td className="px-5 py-3.5 text-xs text-text-muted">{item.decidedAt}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
