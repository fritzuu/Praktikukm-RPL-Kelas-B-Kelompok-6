...
export default function AslabAiAssistantPanel({ isOpen, onClose, ref }) {
    const [chatInput, setChatInput] = useState('');
    const [messages, setMessages] = useState([
        {
            role: 'assistant',
            text: 'Halo Aslab! Saya AI Assistant SARS. Saya siap membantu Anda melakukan validasi request jadwal, menganalisis konflik bentrok ruangan/dosen, serta memantau data jadwal aktif. Silakan tanyakan sesuatu!',
        },
    ]);
    ...
    async function handleSend() {
        ...
        try {
            ...
            const res = await fetch(route('aslab.aiQuery'), {
                method: 'POST',
                headers: {
            ...
            <div className="w-[320px] flex flex-col h-full shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span className="text-[10px] font-semibold text-success uppercase tracking-wide">
                                {isStreaming ? 'Thinking...' : 'Aslab Mode'}
                            </span>
                        </div>
                    </div>
                    <button
                ...
                <div className="px-4 py-2 border-t border-border">
                    <div className="flex gap-1.5 overflow-x-auto">
                        {['Ada request pending?', 'Cek konflik jadwal?', 'Tampilkan statistik request'].map((q, i) => (
                            <button
                                key={i}
