...
import { User, Camera, Shield, Moon, Sun, Monitor, CheckCircle, AlertCircle, Save, Image as ImageIcon } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import AslabLayout from '../../Layouts/AslabLayout';

export default function AslabSetting({ user }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('aslab.pengaturan.update'), { forceFormData: true, preserveScroll: true });
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                    <div className="bg-card border border-border rounded-3xl p-6 text-center shadow-sm">
                        <div className="relative w-32 h-32 mx-auto mb-4 group">
                            <div className="w-full h-full rounded-full overflow-hidden border-4 border-white shadow-xl bg-surface">
                                {previewUrl ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center text-text-muted"><User size={48} /></div>
                                )}
                            </div>
                        ...
                        <p className="text-text-muted text-[10px] font-black uppercase tracking-widest mt-1">Asisten Lab</p>

                        <div className="mt-6 pt-6 border-t border-border space-y-3">
                            <div className="flex items-center gap-3 text-left">
                                <div className="w-8 h-8 rounded-lg bg-surface flex items-center justify-center text-text-secondary"><Shield size={14} /></div>
                                <div>
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <motion.form initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit}
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-lg bg-primary-500/10 flex items-center justify-center text-primary-500"><ImageIcon size={18} /></div>
                        ...
                        <div className="grid grid-cols-1 gap-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2 opacity-70">
                                    ...
                                    <input type="email" value={user.email} disabled className="w-full px-4 py-3 bg-surface/50 border border-border rounded-xl text-text-muted cursor-not-allowed font-medium" />
                                </div>
                                <div className="space-y-2 opacity-70">
                                    <label className="text-[10px] font-black text-text-muted uppercase tracking-widest px-1">NIM / Identitas</label>
                                    <input type="text" value={user.nim_nip} disabled className="w-full px-4 py-3 bg-surface/50 border border-border rounded-xl text-text-muted cursor-not-allowed font-medium" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="pt-4 flex justify-end">
                            <button type="submit" disabled={processing}
                                ...
                                {processing ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Save size={18} />}
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                        ...
                        <div className="flex items-center gap-3 mb-2">
                            <div className="w-8 h-8 rounded-lg bg-primary-500/10 flex items-center justify-center text-primary-500"><Monitor size={18} /></div>
                            ...
                            {[
                                ...
                                <button key={theme.id} type="button" onClick={() => handleThemeChange(theme.id)}
                                    className={`flex flex-col items-center justify-center gap-3 p-6 rounded-2xl border transition-all duration-300
                                        ${activeTheme === theme.id ? 'bg-primary-500/10 border-primary-500/40 text-primary-500 shadow-md scale-[1.02]' : 'bg-surface border-border text-text-muted hover:border-primary-500/20 hover:text-text-primary'}`}>
...
AslabSetting.layout = (page) => <AslabLayout>{page}</AslabLayout>;
