...
class CampusActivityService
{
    ...
    public function buildPayload(?Semester $semester): array
    {
        ...
        $futureOccurrences = $occurrences->filter(
            fn (array $o) => $o['start_time'] > $currentTime
        );

        // If no classes are currently active AND no classes are scheduled later today,
        // all academic activity for the day is effectively over — show empty state.
        // This prevents rooms from appearing as "Available Until 20:15" when there are
        // no remaining lectures, even though we're technically within the session window.
        if ($activeOccurrences->isEmpty() && $futureOccurrences->isEmpty()) {
            return $this->outsideLectureHoursPayload($semester, $now, $todayDow, $currentTime);
        }
